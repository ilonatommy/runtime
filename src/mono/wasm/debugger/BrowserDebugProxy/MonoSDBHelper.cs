...
namespace Microsoft.WebAssembly.Diagnostics
{
    ...
    internal class ValueTypeClass
    {
        ...
        public async Task<JArray> GetValueTypeValues(bool accessorPropertiesOnly, CancellationToken token)
        {
            ...
            if (accessorPropertiesOnly)
                return jsonProps;
            return new JArray(json.Union(jsonProps));
        }
    }
    ...
    internal class MonoSDBHelper
    {
        ...
        public async Task<JArray> GetPropertiesValuesOfValueType(int valueTypeId, CancellationToken token)
        {
            ...
            for (int i = 0 ; i < typesToGetProperties.Count; i++)
            {
                var properties = await CreateJArrayForProperties(typesToGetProperties[i], ElementType.ValueType, valueType.buffer, valueType.json, valueType.autoExpand, $"dotnet:valuetype:{valueType.Id}", isOwn: i == 0, token);
                ret = new JArray(ret.Union(properties));
            }
