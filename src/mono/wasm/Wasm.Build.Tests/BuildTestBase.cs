...
namespace Wasm.Build.Tests
{
    public abstract class BuildTestBase : IClassFixture<SharedBuildPerTestClassFixture>, IDisposable
    {
        ...
        */
                                  ...
                                  BuildProjectOptions options)
        {
            ...
            try
            {
                ...
                if (options.ExpectSuccess && options.AssertAppBundle)
                {
                    ...
                    AssertBasicAppBundle(bundleDir, buildArgs.ProjectName, buildArgs.Config, options.MainJS ?? "test-main.js", options.HasV8Script, options.IcuCultures ?? new string [0], options.IcuFeatures ?? new string [0], options.HasInvariantGlobalization, options.DotnetWasmFromRuntimePack ?? !buildArgs.AOT, options.TargetFramework ?? DefaultTargetFramework);
                }
        ...
        protected static void AssertBasicAppBundle(string bundleDir, string projectName, string config, string mainJS, bool hasV8Script, string[] icuCultures, string[] icuFeatures, bool hasInvariantGlobalization=false, bool dotnetWasmFromRuntimePack=true, string targetFramework=DefaultTargetFramework)
        {
