...
namespace Wasm.Build.Tests
{
    public abstract class BuildTestBase : IClassFixture<SharedBuildPerTestClassFixture>, IDisposable
    {
        ...
        internal BuildPaths GetBuildPaths(ProjectInfo buildArgs, bool forPublish = true)
        {
            string objDir = GetObjDir(buildArgs.Configuration);
            string bundleDir = Path.Combine(GetBinDir(baseDir: _projectDir, config: buildArgs.Configuration), "AppBundle");
            string wasmDir = Path.Combine(objDir, "wasm", forPublish ? "for-publish" : "for-build");

            return new BuildPaths(wasmDir, objDir, GetBinDir(buildArgs.Configuration), bundleDir);
        }
