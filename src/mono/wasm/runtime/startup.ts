...
import { mono_assert, CharPtrNull, DotnetModule, MonoConfig, MonoConfigError, LoadingResource, AssetEntry, ResourceRequest,IcuDictionary } from "./types";
...
function _instantiate_asset(asset: AssetEntry, url: string, bytes: Uint8Array) {
    ...
    if (asset.behavior === "assembly") {
        ...
        if (!mono_wasm_load_icu_data(offset!, +(asset.dataType == "common")))
...
export async function mono_wasm_load_config(configFilePath: string): Promise<void> {
    ...
    try {
        ...
        Module.print("config.application_culture: " + config.applicationCulture);
        const args: any = config;
        const icu_files = _get_list_of_icu_files(config.icuDictionary, config.applicationCulture);
